...
pub fn CountingWriter(comptime Writer: type) type {
    return struct {
        ...
        pub const Self = @This();
        pub const ProxyWriter = struct {
            parent: *Self,
            pub fn writeAll(self: ProxyWriter, bytes: []const u8) !void {
                try self.parent.child_writer.writeAll(bytes);
                self.parent.bytes_written += bytes.len;
            }
            pub fn writeInt(self: ProxyWriter, comptime IntType: type, value: IntType, endian: std.builtin.Endian) !void {
                var bytes: [@sizeOf(IntType)]u8 = undefined;
                std.mem.writeInt(IntType, &bytes, value, endian);
                try self.writeAll(&bytes);
            }
            pub fn writeStruct(self: ProxyWriter, value: anytype, endian: std.builtin.Endian) !void {
                var copy = value;
                if (endian != @import("builtin").target.cpu.arch.endian()) {
                    std.mem.byteSwapAllFields(@TypeOf(value), &copy);
                }
                try self.parent.child_writer.writeAll(std.mem.asBytes(&copy));
                self.parent.bytes_written += @sizeOf(@TypeOf(value));
            }
            pub fn writeStructEndian(self: ProxyWriter, value: anytype, endian: std.builtin.Endian) !void {
                try self.writeStruct(value, endian);
            }
        };

        pub fn writer(self: *Self) ProxyWriter {
            return .{ .parent = self };
        }
    };
}
...
pub fn initZipper(writer: anytype) Zipper(@TypeOf(writer)) {
    return .{ .counting_writer = .{ .child_writer = writer } };
}
...
pub fn Zipper(comptime Writer: type) type {
    return struct {
        counting_writer: CountingWriter(Writer),
        central_count: u64 = 0,
        ...
        pub fn writeFile(
            ...
            switch (opt.compression) {
                ...
                .deflate => {
                    ...
                    var comp = try std.compress.flate.Compress.init(self.counting_writer.child_writer, &window_buf, .raw, std.compress.flate.Compress.Options.level_6);
                    try comp.writer.writeAll(opt.content);
                    try comp.finish();
...
pub const StoredEntry = struct { name: []const u8, data: []const u8 };
pub fn writeStoredZip(alloc: std.mem.Allocator, entries: []const StoredEntry) ![]u8 {
    var total: usize = 22;
    for (entries) |e| total += 30 + e.name.len + e.data.len + 46 + e.name.len;
    const buf = try alloc.alloc(u8, total);
    errdefer alloc.free(buf);
    const store = try alloc.alloc(FileStore, entries.len);
    defer alloc.free(store);
    const files = try alloc.alloc(File, entries.len);
    defer alloc.free(files);
    for (entries, 0..) |e, i| files[i] = .{ .name = e.name, .content = e.data, .compression = .store };
    var writer = std.Io.Writer.fixed(buf);
    try writeZip(&writer, files, store, .{});
